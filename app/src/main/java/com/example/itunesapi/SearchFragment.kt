        ...
        button.setOnClickListener {
            Thread {
                try {
                    ...
                    val baseUrl = "https://itunes.apple.com/search?media=music&entity=song&country=kr"
                    ...
                    val sortedResults = when {
                        searchAttribute.contains("artistTerm", ignoreCase = true) -> {
                            ...
                            // 아래에서 artist name이 포함되지 않으면 제외
                            val filteredResults = (exact + contains).distinct()

                            // 만약 정말로 아무것도 매칭이 안 됐다면 fallback으로 전체 보여주기
                            if (filteredResults.isEmpty()) {
                                allResults.sortedBy { it.artist }
                            } else {
                                filteredResults
                            }
                        }
                            ...
                            val word = allResults.filter {
                                !exact.contains(it) && it.title.lowercase().split(Regex("[\\s\\-]")).contains(rawInput.lowercase())
                            }
                            val partial = allResults.filter {
                                !exact.contains(it) && !word.contains(it) && it.title.lowercase().contains(rawInput.lowercase())
                            }
                        ...
                        searchAttribute.contains("albumTerm", ignoreCase = true) -> {
                            ...
                            val word = allResults.filter {
                                !exact.contains(it) && it.album.lowercase().split(Regex("[\\s\\-]")).contains(rawInput.lowercase())
                            }
                            val partial = allResults.filter {
                                !exact.contains(it) && !word.contains(it) && it.album.lowercase().contains(rawInput.lowercase())
                            }
                            val others = allResults - exact - word - partial
